...
char *Cstrstr (const char* phaystack, const char* pneedle);
char* strstra(const char *str,char c);
unsigned int magic_bits = 0x7efefeffL;

inline int haszerobyte(unsigned int a)
{
...
inline long long haszerobytel(unsigned long long a)
{
...
finda:
		...
		if(reta!=0 ) {
			...
			//if((retb != 0/*contain b*/)&&((reta | retb)&((reta| retb)>>8)/*ab|ba*/ )){
...
char* lstrstr5(char* text, char* pattern)
{
	unsigned int * intPtr = (unsigned int *) text;
	unsigned char * chPtrAligned = text;
	unsigned int intWord  ;//= *intPtr ;
	char chara = pattern[0];
	...
	if(pattern[1] == 0) return strstra(text,pattern[0]); 
	bytePtr[0]=bytePtr[1]=bytePtr[2]=bytePtr[3]=pattern[0];
	bytePtr = (char*) &byte4b0;
	bytePtr[0]=bytePtr[1]=bytePtr[2]=bytePtr[3]=pattern[1];
	byte4a = byte4a0;
...
#if 1
	{
		...
		chPtrAligned = text + preBytes;
		//printf("preBytes:%d\n",preBytes);
		switch(preBytes){
			case 1:
				if((text[0])&&(text[0] == chara)){
					if(strcmpInline(text+1,pattern +1) == 0) return 0;
				}
				break;
			case 2:
				if((text[0])&&(text[0] == chara)){
				...
				if((text[1])&&(text[1] == chara)){
					if(strcmpInline(text+1,pattern +1) == 0) return 1;
				}
				break;
			case 3:
				{
					...
					if(((reta | retb)&((reta| retb)>>8)/*ab|ba|aa|bb*/ )){
						...
						if((text[1])&&(text[1] == chara)){
							if(strcmpInline(text+2,pattern +2) == 0) return 1;
						}
						if((text[2])&&(text[2] == chara)){
							if(strcmpInline(text+3,pattern +3) == 0) return 2;
						}
					}
				}
				break;
...
searcha:
		unsigned int reta = haszerobyte(intWord ^ byte4a);
		if(reta!=0 ) {
findouta:		//unsigned int nextWord = *(intPtr+1);
			unsigned int retb = haszerobyte(intWord ^ byte4b) ;
findoutb:
			//! if((retb != 0/*contain b*/)&&((reta | retb)&((reta| retb)>>8)/*ab|ba*/ )){
			if(((reta | retb)&((reta| retb)>>8)/*ab|ba*/ )){
